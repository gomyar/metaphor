...
class LRParseTest(unittest.TestCase):
    ...
    def perform_simple_calc(self, resource_id, calc):
        val = list(self.db.metaphor_resource.aggregate([{"$match": {"_id": self.schema.decodeid(resource_id)}}] + calc.create_aggregation()))
        return val[0]['_val'] if val else None

    def perform_resource_calc(self, resource_id, calc):
        val = list(self.db.metaphor_resource.aggregate([{"$match": {"_id": self.schema.decodeid(resource_id)}}] + calc.create_aggregation()))
        return val
    ...
    def test_field(self):
        ...
        self.assertEqual('Bob', self.perform_simple_calc(self.employee_id_1, tree))

    def test_calc(self):
        tree = parse("self.age + self.duration", self.employee_spec)

        self.assertEqual(56, self.perform_simple_calc(self.employee_id_1, tree))
        self.assertEqual(42, self.perform_simple_calc(self.employee_id_2, tree))

    def test_linked_resource(self):
        tree = parse("self.boss", self.employee_spec)

        val = self.perform_resource_calc(self.employee_id_2, tree)
    ...
    def test_linked_calc(self):
        ...
        self.assertEqual(46, self.perform_simple_calc(self.employee_id_2, tree))
        self.assertEqual(36, self.perform_simple_calc(self.employee_id_3, tree))
    ...
    def test_ternary(self):
        ...
        self.assertEqual(12, self.perform_simple_calc(self.employee_id_1, tree))

        self.assertEqual(14, self.perform_simple_calc(self.employee_id_2, tree))

        self.assertEqual(14, self.perform_simple_calc(self.employee_id_3, tree))

    def test_ternary_calcs(self):
        tree = parse("self.boss.name = 'Bob' -> (self.boss.duration) : 99", self.employee_spec)
        val = self.perform_simple_calc(self.employee_id_1, tree)
        self.assertEqual(99, val)

        val = self.perform_simple_calc(self.employee_id_2, tree)
        self.assertEqual(12, val)

        val = self.perform_simple_calc(self.employee_id_3, tree)
        self.assertEqual(12, val)

    def test_switch(self):
        tree = parse("self.name -> ('Bob': 22, 'Ned': 11, 'Fred': 4)", self.employee_spec)

        self.assertEqual(22, self.perform_simple_calc(self.employee_id_1, tree))

        self.assertEqual(11, self.perform_simple_calc(self.employee_id_2, tree))

        self.assertEqual(None, self.perform_simple_calc(self.employee_id_3, tree))

    def test_switch_calc(self):
        tree = parse("self.boss.name -> ('Bob': 22, 'Ned': 11, 'Fred': 4)", self.employee_spec)

        self.assertEqual(None, self.perform_simple_calc(self.employee_id_1, tree))

        self.assertEqual(22, self.perform_simple_calc(self.employee_id_2, tree))

        self.assertEqual(22, self.perform_simple_calc(self.employee_id_3, tree))
    ...
    def test_switch_calc_fields(self):
        ...
        self.assertEqual(12, self.perform_simple_calc(self.employee_id_2, tree))

        self.assertEqual(12, self.perform_simple_calc(self.employee_id_3, tree))
    ...
    def test_function_sum(self):
        ...
        self.assertEqual(102, self.perform_simple_calc(self.employee_id_1, tree))
